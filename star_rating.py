...
def get_obscurity(text: str) -> float:
    ...
    for i, char in enumerate(text):
        ...
        new = next((
            item for item in combos if item['Combo'] == combo
        ), None)
        ...
        a = (new['Count'] / 400) ** .1 # Change values if you use a different combo set
...
def get_density(text: str) -> float:
    ...
    for i, char in enumerate(text):
        ...
        first = next((
            item for item in symbol_ratings if item[0] == char
        ), None)

        second = next((
            item for item in symbol_ratings if item[0] == after
        ), None)
    ...
    # Map
    density = 3.8 * (score / len(text) * 10) ** .4

    return density


def get_star(text: str) -> float:
    calc = (0.45 * get_length(text)) + (.27 * get_obscurity(text)) + (.28 * get_density(text))
    star = calc ** 1.2
    return star
